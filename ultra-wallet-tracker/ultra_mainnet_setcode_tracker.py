...
DB_FILE = os.path.join("data", "setcode_mainnet.db")
EXPORT_DIR = "daily_exports/mainnet"
...
def plot_by_category():
    ...
    out_csv = os.path.join("data", "setcode_mainnet_by_category.csv")
    out_png = os.path.join("exports", "setcode_mainnet_by_category.png")
    pivot.to_csv(out_csv)

    plt.figure(figsize=(12, 6))
    pivot.plot.area(stacked=True, ax=plt.gca())
    plt.title("Mainnet Contract Deployments by Category")
    plt.xlabel("Date")
    plt.ylabel("Deployments")
    plt.tight_layout()
    plt.savefig(out_png)
    plt.close()
    print(f"📊 Saved mainnet category chart to {out_png}")

# === PLOT DAILY COUNTS ===
def plot_daily_activity():
    conn = sqlite3.connect(DB_FILE)
    df = pd.read_sql_query("SELECT timestamp FROM setcode", conn)
    df["date"] = pd.to_datetime(df["timestamp"]).dt.date
    daily_counts = df.groupby("date").size().reset_index(name="contracts_deployed")

    out_csv = os.path.join("data", "setcode_mainnet_timeseries.csv")
    out_png = os.path.join("exports", "setcode_mainnet_activity.png")
    daily_counts.to_csv(out_csv, index=False)

    plt.figure(figsize=(10, 5))
    plt.plot(daily_counts["date"], daily_counts["contracts_deployed"], marker="o")
    plt.title("Mainnet Contract Deployments Per Day (`eosio:setcode`)")
    plt.xlabel("Date")
    plt.ylabel("Contracts Deployed")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(out_png)
    plt.close()
    print(f"📈 Saved mainnet daily activity chart to {out_png}")

# === MAIN ===
if __name__ == "__main__":
    print("🔄 Ultra Mainnet Contract Tracker (Enriched)")
    sync_setcode()
    plot_by_category()
    plot_daily_activity()
